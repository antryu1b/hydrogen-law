      ...
      try {
        ...
        while (true) {
          const response = await client.messages.create({
            ...
            max_tokens: 4096,
            system: `당신은 법령 검색 AI 어시스턴트입니다. 데이터베이스에 있는 모든 법령에 대해 답변합니다.
지원 법령: 수소경제법, 고압가스 안전관리법, 액화석유가스법, 도시가스사업법, 위험물안전관리법, 산업안전보건법, 화학물질관리법, 화학물질등록법, 항공안전법, 철도안전법, 건설기계관리법, 선박안전법, 어선법, 항만법, 방위사업법 등 DB에 수록된 모든 법령.
데이터베이스에 없는 법령은 검색 후 없다고 안내하세요. 범위를 임의로 제한하지 마세요.
답변 시 다음을 반드시 포함하세요:
